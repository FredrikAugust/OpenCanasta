defmodule CanastaMeldTest do
  ...
  describe "create_meld/3" do
    ...
    test "validate_ratio/2", state do
      assert {:error, ["You must have more natural cards than wild cards."]} ==
               Meld.create_meld(state.invalid_ratio.cards)

      assert {:error,
              [
                "You must have one type of natural card in a meld.",
                "You must have more natural cards than wild cards."
              ]} == Meld.create_meld(state.invalid_final_meld.cards, true)
      ...
      assert {:error,
              [
                "You must have one type of natural card in a meld.",
                "You must have more natural cards than wild cards."
              ]} == Meld.create_meld(state.final_meld.cards)
    end

    test "validate_purity/1", state do
      assert {:error, ["You must have one type of natural card in a meld."]} ==
               Meld.create_meld(state.invalid_purity.cards)
    end

    test "validate_number_of_cards/1", state do
      assert {:error, ["You need at least 3 cards in a meld."]} ==
               Meld.create_meld(state.invalid_card_number.cards)
    end
  end
